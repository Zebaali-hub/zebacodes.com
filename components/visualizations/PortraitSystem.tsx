...
const nodes = [
  { label: 'API', detail: 'request', className: 'portrait-node node-api' },
  { label: 'SERVICE', detail: 'logic', className: 'portrait-node node-service' },
  { label: 'EVENT', detail: 'async', className: 'portrait-node node-event' },
  { label: 'DATA', detail: 'persist', className: 'portrait-node node-db' },
  { label: 'CACHE', detail: 'fast path', className: 'portrait-node node-cache' },
]
...
export default function PortraitSystem() {
  ...
  function move(event: React.PointerEvent<HTMLDivElement>) {
    ...
    frame.current?.style.setProperty('--rx', `${-y * 3}deg`)
    frame.current?.style.setProperty('--ry', `${x * 3}deg`)
    frame.current?.style.setProperty('--px', `${x}`)
    frame.current?.style.setProperty('--py', `${y}`)
  }
  function reset() {
    frame.current?.style.setProperty('--rx', '0deg')
    frame.current?.style.setProperty('--ry', '0deg')
    frame.current?.style.setProperty('--px', '0')
    frame.current?.style.setProperty('--py', '0')
  }
  return <div ref={frame} className="portrait-system" onPointerMove={move} onPointerLeave={reset}>
    ...
    {nodes.map((node) => <span key={node.label} className={node.className}><strong>{node.label}</strong><small>{node.detail}</small></span>)}
