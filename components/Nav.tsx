...
const links = [
  { label: 'Home', href: '/' }, { label: 'Projects', href: '/#systems' }, { label: 'Writing', href: '/writing' },
...
export default function Nav() {
  ...
  return <header className="site-nav"><nav className="site-container" aria-label="Primary navigation">
    ...
    <div id="mobile-navigation" className={`nav-links ${open ? 'is-open' : ''}`}>{links.map((link) => <Link key={link.label} href={link.href} onClick={() => setOpen(false)}>{link.label}</Link>)}<a className="nav-github" href="https://github.com/Zebaali-hub" target="_blank" rel="noreferrer"><Code2 size={14} /> GitHub ↗</a></div>
